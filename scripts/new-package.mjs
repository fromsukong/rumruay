...
if (!name) {
  ...
  console.error('Example: pnpm new budgets');
  process.exit(1);
}

if (!/^[a-z][a-z0-9-]*$/.test(name)) {
  console.error(`Invalid name "${name}". Use kebab-case lowercase (e.g., "budgets", "reports")`);
  process.exit(1);
}

const scoped = `@rumruay/${name}`;
...
const packageJson = {
  name: scoped,
  version: '0.1.0',
  description: `${titleCase} module for personal finance, built on @rumruay/core.`,
  type: 'module',
  license: 'MIT',
  sideEffects: false,
  engines: { node: '>=18' },
